    ...
    <div className="space-y-6 pb-24 animate-in fade-in duration-500">
      {trips.length === 0 ? (
        ...
        <div className="space-y-3">
          {trips.map((trip) => (
            <div key={trip.id} className="bg-zinc-900/40 rounded-3xl overflow-hidden border border-white/5">
              <div
                className="group relative p-4 flex justify-between items-center active:bg-white/5 transition-colors"
              >
                {confirmingId === trip.id && (
                  <div className="absolute inset-0 bg-black/95 z-30 flex items-center justify-between px-6 animate-in slide-in-from-right duration-200">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => confirmDelete(trip.id)} className="px-4 py-2 bg-white text-black rounded-full text-sm font-bold">Vymazať</button>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <div className="text-[10px] font-medium text-zinc-600 uppercase tabular-nums">{trip.endOdometer} km</div>
                  </div>
                  <button
